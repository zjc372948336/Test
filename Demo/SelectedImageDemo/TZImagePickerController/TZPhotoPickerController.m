...
#define SHOW_COLLECTIONVIEW_HEIGHT 92.f

@interface TZPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,TZRightPicCellDelegate,TZShowCellDelegate> {
...
@property (nonatomic, strong) TZCollectionView *collectionView;//图片collectionView
@property (nonatomic, strong) UIImagePickerController *imagePickerVc;
@property (nonatomic, strong) UICollectionView *showCollectionView;//选择完显示图片collectionview
@end
...
- (void)configCollectionView {
    ...
    _collectionView = [[TZCollectionView alloc] initWithFrame:CGRectMake(0, top, self.view.tz_width, collectionViewHeight - SHOW_COLLECTIONVIEW_HEIGHT + 50) collectionViewLayout:layout];
    ...
    [_collectionView registerClass:[TZRightPicCell class] forCellWithReuseIdentifier:@"TZRightPicCell"];
...
- (void)configBottomToolBar {//底下展示选中图片
    
    UICollectionViewFlowLayout *showLayout = [[UICollectionViewFlowLayout alloc] init];
    showLayout.minimumInteritemSpacing = 5;
    showLayout.minimumLineSpacing = 0;
    [showLayout setScrollDirection:UICollectionViewScrollDirectionHorizontal];
    
    _showCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0,_collectionView.frame.origin.y + _collectionView.frame.size.height, self.view.tz_width - 92, SHOW_COLLECTIONVIEW_HEIGHT) collectionViewLayout:showLayout];
    _showCollectionView.backgroundColor = [UIColor blackColor];
    _showCollectionView.dataSource = self;
    _showCollectionView.delegate = self;
    _showCollectionView.contentInset = UIEdgeInsetsMake(5, 5, 5, 5);
    [_showCollectionView registerClass:[TZShowCell class] forCellWithReuseIdentifier:@"TZShowCell"];
    [self.view addSubview:_showCollectionView];
    
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(self.view.tz_width - 92,_collectionView.frame.origin.y + _collectionView.frame.size.height, 92, 92)];
...
//    }
    ...
    _doneButton.frame = CGRectMake(11, 11, 70, 70);
...
//    _doneButton.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
    _doneButton.backgroundColor = UIColorFromRGB(0xd61c1c);
    
    _numberLabel = [[UILabel alloc] init];
    _numberLabel.frame = _doneButton.frame;
    _numberLabel.font = [UIFont systemFontOfSize:17];
    _numberLabel.textColor = [UIColor whiteColor];
    _numberLabel.numberOfLines = 0;
    _numberLabel.textAlignment = NSTextAlignmentCenter;
    _numberLabel.text = [NSString stringWithFormat:@"确定\n%zd/9",tzImagePickerVc.selectedModels.count];
...
-(void)doneButtonClickAction{//完成按钮
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    ...
    if (tzImagePickerVc.selectedModels.count <= 0) {
        ...
        UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@
                                                                           "提示"
                                                                           message:@"选择图片张数必须大于0" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)doneButtonClick {//详细图片里面的完成按钮
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    // 1.6.8 判断是否满足最小必选张数的限制
    if (tzImagePickerVc.minImagesCount && tzImagePickerVc.selectedModels.count < tzImagePickerVc.minImagesCount) {
        NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"Select a minimum of %zd photos"], tzImagePickerVc.minImagesCount];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (collectionView == _showCollectionView) {
        TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
        return tzImagePickerVc.selectedModels.count >= 9 ? 9:tzImagePickerVc.selectedModels.count + 1;
    }else{
        NSLog(@"%ld",_model.count);
        TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
        if(section == 1){
            if (tzImagePickerVc.allowPickingImage && tzImagePickerVc.allowTakePicture) {
                return _models.count - 4;
            }
        }else{
            return 2;
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _showCollectionView) {
        TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
        TZShowCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZShowCell" forIndexPath:indexPath];
        if (tzImagePickerVc.selectedModels.count  < 9 && indexPath.row == tzImagePickerVc.selectedModels.count) {
            cell.deleteBtn.hidden = YES;
            cell.imageView.image = nil;
        }else{
            cell.deleteBtn.hidden = NO;
            cell.model = tzImagePickerVc.selectedModels[indexPath.row];
        }
        ...
        if (indexPath.section == 1) {
            ...
            if (tzImagePickerVc.sortAscendingByModificationDate || !_showTakePhotoBtn) {
                model = _models[indexPath.row + 4];
            } else {
                model = _models[indexPath.row + 3];
            }
...
//                [UIView showOscillatoryAnimationWithLayer:weakLayer type:TZOscillatoryAnimationToSmaller];
            ...
            if (indexPath.row == 0) {//拍照按钮
                ...
                TZRightPicCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"TZRightPicCell" forIndexPath:indexPath];
                if (_models.count < 4) {
                    switch (_models.count) {
                        case 1:
                            cell.modelArray = [NSArray arrayWithObjects:_models[0], nil];
                            break;
                        case 2:
                            cell.modelArray = [NSArray arrayWithObjects:_models[0],_models[1], nil];
                            break;
                        case 3:
                            cell.modelArray = [NSArray arrayWithObjects:_models[0],_models[1],_models[2], nil];
                            break;
                        default:
                            break;
                    }
                    ...
                    cell.modelArray = [NSArray arrayWithObjects:_models[0],_models[1],_models[2],_models[3], nil];
                }
                cell.photoDefImageName = tzImagePickerVc.photoDefImageName;
                cell.photoSelImageName = tzImagePickerVc.photoSelImageName;
                cell.showSelectBtn = tzImagePickerVc.showSelectBtn;
//                cell.firstImageView.backgroundColor = [UIColor redColor];
//                cell.secondImageView.backgroundColor = [UIColor greenColor];
//                cell.thirdImageView.backgroundColor = [UIColor blueColor];
//                cell.fourtImageView.backgroundColor = [UIColor yellowColor];
                cell.delegate = self;
                return cell;
            }
        }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _showCollectionView) {
        ...
        if ( tzImagePickerVc.selectedModels.count  < 9 && indexPath.row == tzImagePickerVc.selectedModels.count) {
            ...
            [self showCollectionViewImageWithIndex:indexPath.row];
        }
        return;
    }
...
-(void)showCollectionViewImageWithIndex:(NSInteger)index{//查看大图
    ...
    if (model.type == TZAssetModelMediaTypeVideo) {
        ...
        photoPreviewVc.isHiddenSelectedBtn = YES;
        [self pushPhotoPrevireViewController:photoPreviewVc];
    }
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(collectionView == _collectionView){
        if (indexPath.section == 0) {
            size = CGSizeMake((width - (self.columnNumber - 1) * 5)/2,(width - (self.columnNumber - 1) * 5)/2);
        }else{
            size = CGSizeMake((width - (self.columnNumber + 1) * 5)/self.columnNumber,(width - (self.columnNumber + 1) * 5)/self.columnNumber);
        }
    }else{
        size = CGSizeMake(SHOW_COLLECTIONVIEW_HEIGHT - 10,SHOW_COLLECTIONVIEW_HEIGHT - 10);
    }
    return size;
}

- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section{
    if (section == 0 && collectionView == _collectionView) {
...
#pragma mark - TZRightPicCellDelegate
-(void)didSelectPhoto:(BOOL)isSelected withTZRightPicCell:(UICollectionViewCell *)cell andIndex:(NSInteger)index{
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    TZRightPicCell *weakCell = (TZRightPicCell *)cell;
    // 1. cancel select / 取消选择
    if (isSelected) {
        switch (index) {
            case 0:{
                weakCell.firstBtn.selected = NO;
                }
                break;
            case 1:{
                weakCell.secondBtn.selected = NO;
            }
                break;
            case 2:{
                weakCell.thirdBtn.selected = NO;
            }
                break;
            case 3:{
                weakCell.fourtBtn.selected = NO;
            }
                break;
        ...
        ((TZAssetModel *)[_models objectAtIndex:index]).isSelected = NO;
        NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
        for (TZAssetModel *model_item in selectedModels) {
            if ([[[TZImageManager manager] getAssetIdentifier:((TZAssetModel *)[_models objectAtIndex:index]).asset] isEqualToString:[[TZImageManager manager] getAssetIdentifier:model_item.asset]]) {
                [tzImagePickerVc.selectedModels removeObject:model_item];
                break;
            }
        }
        ...
        if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
            switch (index) {
                case 0:{
                    weakCell.firstBtn.selected = YES;
                }
                    break;
                case 1:{
                    weakCell.secondBtn.selected = YES;
                }
                    break;
                case 2:{
                    weakCell.thirdBtn.selected = YES;
                }
                    break;
                case 3:{
                    weakCell.fourtBtn.selected = YES;
                }
                    break;
            ...
            ((TZAssetModel *)[_models objectAtIndex:index]).isSelected = YES;
            [tzImagePickerVc.selectedModels addObject:((TZAssetModel *)[_models objectAtIndex:index])];
            [self refreshBottomToolBarStatus];
        } else {
            NSString *title = [NSString stringWithFormat:[NSBundle tz_localizedStringForKey:@"Select a maximum of %zd photos"], tzImagePickerVc.maxImagesCount];
            [tzImagePickerVc showAlertWithTitle:title];
...
-(void)imageSelectedWithIndex:(NSInteger)index{//查看大图
...
#pragma mark - TZShowCellDelegate
-(void)deleteSelectedImage:(NSInteger)index{//展示选中图片删除
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
...
- (void)refreshBottomToolBarStatus {//刷新底部
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    
    _numberLabel.text = [NSString stringWithFormat:@"确定\n%zd/9",tzImagePickerVc.selectedModels.count];
    [_showCollectionView reloadData];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:tzImagePickerVc.selectedModels.count inSection:0];
    if(tzImagePickerVc.selectedModels.count < 9){
    [_showCollectionView scrollToItemAtIndexPath:indexPath atScrollPosition:UICollectionViewScrollPositionRight animated:YES];
    }
}
...
- (void)checkSelectedModels {//判断图片是否选中
        ...
        [[TZImageManager manager] getAssetsFromFetchResult:_model.result allowPickingVideo:tzImagePickerVc.allowPickingVideo allowPickingImage:tzImagePickerVc.allowPickingImage completion:^(NSArray<TZAssetModel *> *models) {
            ...
            if (tzImagePickerVc.maxImagesCount <= 1) {
                if (tzImagePickerVc.allowCrop) {
                    ...
                    [tzImagePickerVc.selectedAssets addObject:assetModel.asset];
